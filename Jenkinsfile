   ...
   docker.withRegistry("https://registry.hub.docker.com", "private_docker_hub"){
      //echo node-microservice-openshift/books
            stage('Build Docker Images'){
               def books = docker.build("my-image:${env.BUILD_ID}","-f ${env.WORKSPACE}/books/Dockerfile .")
               def search = docker.build("my-image:${env.BUILD_ID}","-f ${env.WORKSPACE}/search/Dockerfile .") 
